package consistenhash

import (
	"hash/crc32"
	"sort"
)

type HashFunc func(data []byte) uint32

// NodeMap stores information about all nodes
type NodeMap struct {
	hashFunc    HashFunc
	nodeHashs   []int
	nodehashMap map[int]string
}

func NewNodeMap(fn HashFunc) *NodeMap {
	m := &NodeMap{
		hashFunc:    fn,
		nodehashMap: make(map[int]string),
	}
	if m.hashFunc == nil {
		m.hashFunc = crc32.ChecksumIEEE
	}
	return m
}

// IsEmpty check whether the cluster is empty
func (m *NodeMap) IsEmpty() bool {
	return len(m.nodehashMap) == 0
}

// AddNode add a new node to the consistent hah ring
func (m *NodeMap) AddNode(keys ...string) {
	for _, key := range keys {
		// if the string is empty
		if key == "" {
			continue
		}
		hash := int(m.hashFunc([]byte(key)))
		m.nodeHashs = append(m.nodeHashs, hash)
		m.nodehashMap[hash] = key
	}
	sort.Ints(m.nodeHashs)
}

// PickNode know that k and v go to those nodes
func (m *NodeMap) PickNode(key string) string {
	if m.IsEmpty() {
		return ""
	}
	hash := int(m.hashFunc([]byte(key)))
	idx := sort.Search(len(m.nodeHashs), func(i int) bool {
		return m.nodeHashs[i] >= hash
	})
	if idx == len(m.nodeHashs) {
		idx = 0
	}
	return m.nodehashMap[m.nodeHashs[idx]]
}
